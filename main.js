...
function loadGfxSettings() {
	try {
		...
		if (raw) {
			...
			return {
				scale: clamp(Number(d.scale) || 1, 0.25, 4),
				shadows: !!d.shadows,
				detail: ['simple', 'medium', 'advanced'].includes(d.detail) ? d.detail : 'medium',
				particles: clamp(Number(d.particles) ?? 100, 0, 100)
			};
		}
	} catch (e) {}
...
function saveGfxSettings() {
	localStorage.setItem(GFX_KEY, JSON.stringify(gfxSettings));
}
...
document.getElementById('gfxResGrid')?.addEventListener('click', (e) => {
	...
	if (!btn) return;
	gfxSettings.scale = parseFloat(btn.dataset.scale);
	saveGfxSettings();
	applyGfxSettings();
	renderGraphicsMenu();
});
...
document.getElementById('gfxParticleSlider')?.addEventListener('input', (e) => {
	gfxSettings.particles = parseInt(e.target.value, 10);
	saveGfxSettings();
	applyGfxSettings();
	const pctEl = document.getElementById('gfxParticlePct');
	const msgEl = document.getElementById('gfxParticleMsg');
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	// ========================
	// On low-end devices, run collisions every N frames
	if (!tick._colFrame) tick._colFrame = 0;
	tick._colFrame++;
	const runCollisions = (tick._colFrame % gfx.collideEvery) === 0;
...
function setupCanvases() {
	...
	function handleResize() {
		...
		width = w / viewScale;
		height = h / viewScale;
		// Cap extreme scales to avoid GPU OOM on weak devices
		const maxDim = 4096;
	...
	function frameHandler(timestamp) {
		...
		const drawScale = (canvas.width / window.innerWidth) * viewScale;
...
const GAME_CACHE = 'cube-crash-offline-v1.7.1';
...
async function areAssetsReady() {
	if (localStorage.getItem(ASSETS_READY_KEY) === '1') {
		// Verify cache still has music (critical offline)
		try {
			const cache = await caches.open(GAME_CACHE);
			const music = await cache.match('./data/sounds/music.mp3');
